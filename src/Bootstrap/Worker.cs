...
internal class Worker(IServiceProvider serviceProvider) : IHostedService
{
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        var installationService = scope.ServiceProvider.GetRequiredService<InstallationService>();

        var installation = await installationService.FindOrCreateInstallationForAgent(TimingData.Installation.AgentId);
        foreach (var timingpoint in TimingData.timingPoints)
        {
            var tp = await installationService.FindOrCreateTimingPoint(installation.AgentId, timingpoint.SystemId);
        }
