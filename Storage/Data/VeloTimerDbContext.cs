    ...
    public class VeloTimerDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            builder.Entity<Activity>(x =>
            {
                ...
                x.HasMany(x => x.Sessions).WithOne(x => x.Activity);
                x.HasOne(x => x.Transponder).WithMany().IsRequired();
            });
            builder.Entity<Session>(x =>
            {
                x.Property(p => p.Start)
                    .HasConversion(
                        v => v,
                ...
                x.Property(p => p.End)
                    .HasConversion(
                        ...
                        v => new DateTime(v.Ticks, DateTimeKind.Utc))
                    .IsRequired();
                x.HasOne(x => x.Activity).WithMany(x => x.Sessions).IsRequired();
            });
